...
@router.get("/prescriptions/{pid}/document", summary="Официальный документ предписания (HTML для печати/PDF)")
def prescription_document(pid: str, repo: StoreDep, user: User = Depends(get_current_user)):
    ...
    # Нарушения = пункты «issue» из связанной проверки → названия из чек-листа.
    ...
    if insp is None:
        insp = next((i for i in inspections if i.objectId == pr.objectId
                     and any(str(c.value) == "issue" or str(getattr(c.value, "value", "")) == "issue" for c in i.checklist)), None)
    violations = []
    if insp:
        violations = [template.get(c.key, c.label) for c in insp.checklist
                      if str(c.value) == "issue" or str(getattr(c.value, "value", "")) == "issue"]
    ...
    html = render_html(city=city, presc=pr, obj=obj, owner=owner,
